...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const instructions = data.instructions || null; // Get instructions from request

    console.log("Processing message length:", message.length);
    console.log("Stream mode:", streamMode);
    console.log("Language:", language);
    if (instructions)
      console.log("Special instructions provided:", instructions);
    ...
    let systemMessage = `You are an Expert Educational Content Creator with a warm, engaging teaching style. Analyze the following content and create comprehensive, well-structured educational notes that sound conversational and human.`;

    // Check if we have special instructions that should override default guidance
    if (instructions && instructions.trim()) {
      // Special user instructions exist, so use a simplified system message that prioritizes them
      systemMessage += `\n\nIMPORTANT - USER INSTRUCTIONS: ${instructions.trim()}\n\nThe above instructions override any default formatting or structure guidelines below.`;

      // Add baseline format as fallback but clearly indicate user instructions take precedence
      systemMessage += `\n\nIf the user instructions don't specify a structure, you can fall back to this format:
1. Start each section with a clear title
2. Include an introduction to the topic
3. Present concepts in a logical order
4. Use examples to illustrate complex ideas
5. End with a summary of key points

Each section should have a title and at least one sentence. All sentences must have an id, text content, and format. Respond in ${language}.`;
    } else {
      // No special instructions, use the detailed default system message
            ...
            for (const chunk of textChunks) {
              ...
              for await (const part of stream) {
                if (part.choices[0]?.delta?.function_call?.arguments) {
                  ...
                  if (
                    argumentChunk.includes("title") ||
                    argumentChunk.includes("sentences")
                  ) {
                    ...
                    if (match) {
                      for (const sectionStr of match) {
                        try {
                          ...
                          console.log(
                            "Potential section found:",
                            sectionJson.substring(0, 200) + "..."
                          );
                          ...
                          try {
                            ...
                            if (
                              ...
                              parsed.section.sentences.every(
                                (s: Sentence) =>
                                  typeof s.id === "number" &&
                                ...
                                JSON.stringify(
                                  {
                                    ...
                                    sentenceCount:
                                      parsed.section.sentences.length,
                              ...
                              console.error(
                                "Found section with validation issues:",
                                parsed.section
                              );
              ...
              if (functionCallBuffer) {
                ...
                console.log(
                  `- Buffer size: ${functionCallBuffer.length} bytes`
                );
                console.log(
                  `- Buffer starts with: ${functionCallBuffer.substring(
                    0,
                    100
                  )}...`
                );
                console.log(
                  `- Buffer ends with: ${functionCallBuffer.substring(
                    functionCallBuffer.length - 100
                  )}`
                );
                ...
                try {
                  ...
                  console.log(
                    "Sanitized buffer (first 300 chars):",
                    cleanedBuffer.substring(0, 300)
                  );
                  ...
                  const openBrackets = (cleanedBuffer.match(/\[/g) || [])
                    .length;
                  const closeBrackets = (cleanedBuffer.match(/\]/g) || [])
                    .length;
                  ...
                  console.log(
                    `- Brace balance: ${openBraces}:${closeBraces}, Bracket balance: ${openBrackets}:${closeBrackets}`
                  );
                  ...
                  try {
                    ...
                    const validSections = bufferObject.sections.filter(
                      (section: any) => {
                    ...
                    console.log(
                      `Found ${bufferObject.sections.length} sections, ${validSections.length} valid after filtering`
                    );
                    ...
                    for (const section of validSections) {
                      ...
                      if (section.sentences) {
                        section.sentences = section.sentences.map(
                          (sentence: Sentence) => {
                            if (sentence.format === "formula") {
                              sentence.text = cleanFormulaContent(
                                sentence.text
                              );
                            }
                            return sentence;
                    ...
                    if (
                      bufferObject.sections.length > 0 &&
                      validSections.length === 0
                    ) {
                      console.log(
                        "All sections were empty, skipping error generation"
                      );
                    }
                  } catch (parseError) {
                    ...
                    const validPartsPattern =
                      /\{\s*"title"\s*:\s*"[^"]+"\s*,\s*"sentences"\s*:\s*\[\s*(?:\{[^\{\}]*\}\s*,\s*)*\{[^\{\}]*\}\s*\]\s*\}/g;
                    ...
                    if (validParts && validParts.length > 0) {
                      console.log(
                        "Found",
                        validParts.length,
                        "valid section objects to salvage"
                      );
                      ...
                      for (const validSection of validParts) {
                        try {
                          ...
                          console.error(
                            "Failed to parse seemingly valid section:",
                            e
                          );
                        }
                      }
                      ...
                      const titleMatch = cleanedBuffer.match(
                        /"title"\s*:\s*"([^"]+)"/
                      );
                      ...
                      const textMatches = cleanedBuffer.match(
                        /"text"\s*:\s*"([^"]{20,})"/g
                      );
                      let contentSentences: Array<{
                        id: number;
                        text: string;
                        format: string;
                      }> = [];
                      ...
                      if (textMatches && textMatches.length > 0) {
                        ...
                        contentSentences = textMatches.map(
