  ...
  const sendMessage = async (messageText: string = input) => {
    ...
    try {
      ...
      setMessages(prevMessages => [...prevMessages, {
        user: 'System',
        text: errorResponse.map(section => section.sentences.map(sentence => sentence.text).join(' ')).join('\n\n')
      }]);
    }
  };
  ...
  return (
    ...
    <div className='flex flex-col h-full  min-w-[400px] bg-slate-800 rounded-2xl mx-4 max-h-[80vh]'>
      <div className='bg-slate-800 text-white rounded-lg  border-red-900 border-2 p-4 m-4'>
        ...
        <div>
        {primeSentence ? <p>{primeSentence}</p> : <p className='text-gray-500'>Click a Sentence to add context</p>}
        </div>
      </div>
      <div className='flex-grow overflow-y-auto p-4 auto-scroll'>
        {messages.map((msg, index) => (
          msg.text && (
            <div key={index} className={`p-2 rounded mb-2 ${msg.user === 'User' ? 'bg-slate-800 text-gray-500' : 'bg-slate-900 shadow'}`}>
              {msg.user === 'AI' ? (
                  ...
                  {Array.isArray(msg.text) ? (
                    <div className="space-y-6 ">
                      {msg.text.map((section: Section, sectionIdx: number) => (
                        <div key={sectionIdx} className="bg-white p-4 rounded-lg shadow">
