...
export const ResponseMessage = ({msg, handleSectionClick, handleSentenceClick, index}: ResponseProps) => {
  return (
    <div key={index} className="p-2 rounded mb-2 bg-slate-200">
      {/* AI Responses */}
      {msg.user === "AI" && (
        <div className="text-sm">
          {msg.text.map((section: Section, sectionIdx: number) => (
            <div
              key={sectionIdx}
              className="bg-slate-200 border border-[#94b347] p-4 rounded-2xl transition-colors"
            >
              <h3
              ...
              <div className="space-y-2">
                {section.sentences.map((sentence: Sentence, sentenceIdx: number) => (
                  <Button
                    ...
                    className="bg-slate-200 hover:border hover:bg-[#e9efda]   rounded cursor-pointer transition-colors shadow-none p-0 m-0"
                  >
